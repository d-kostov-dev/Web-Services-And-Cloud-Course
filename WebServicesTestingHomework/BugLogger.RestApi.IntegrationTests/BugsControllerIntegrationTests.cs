...
namespace BugLogger.RestApi.IntegrationTests
{
    ...
    public class BugsControllerIntegrationTests
    {
        ...
        // My Tests
        [TestMethod]
        public void GetByDate_WhenBugsInDb_ShouldReturnStatus200AndNonEmptyContent()
        {
            IRepository<Bug> repo = Mock.Create<IRepository<Bug>>();
            Bug[] bugs = { new Bug(){LogDate = DateTime.Now,}, new Bug(){LogDate = DateTime.Now,}, new Bug() };
            ...
            var response = server.CreateGetRequest("/api/bugs?date=12-12-2014");
        ...
        [TestMethod]
        public void GetByStatus_WhenBugsInDb_ShouldReturnStatus200AndNonEmptyContent()
        {
            IRepository<Bug> repo = Mock.Create<IRepository<Bug>>();
            Bug[] bugs = { new Bug() { LogDate = DateTime.Now, }, new Bug() { LogDate = DateTime.Now, }, new Bug() };
            ...
            var response = server.CreateGetRequest("/api/bugs?bugStatus=pending");
